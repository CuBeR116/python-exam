...
def printUsers():
    ...
    for index, user in enumerate(users):
        ...
        options = ['Обычный пользователь', 'Админ']
        variable = tk.StringVar(usersWindows)
        variable.set(options[user['is_admin']])

        listBox = tk.OptionMenu(usersWindows, variable, *options, command=lambda event: changePermission(user['id'], event, options))
        listBox.grid(row=index + 3, column=2)
        ...
        removeUserButton.grid(row=index + 3, column=3, sticky="nsew")

def changePermission(user, value, options):
    index = options.index(value)
    query = """
    UPDATE user SET is_admin = ?
    WHERE id = ?
    """
    cursor.execute(query, [index, user])
    sqlite_connection.commit()
...
def resetTable():
    ...
    INSERT INTO user(login, password, is_admin) VALUES ("CuBeR116", "pass", 1);
    """
...
usersWindows = tk.Tk(screenName="Список пользователей")
